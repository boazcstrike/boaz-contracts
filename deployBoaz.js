...
const Token = require('./client/src/contracts/BoazToken.json');
...
async function deployMasterChef(tokenContractAddress, account, nonce) {
    const boazPerBlock = web3.utils.toWei('1', 'ether');

    MasterChefContract.deploy({
        data: MasterChef.bytecode,
        arguments: [tokenContractAddress, account.address, account.address, boazPerBlock, 6347879]
    })
        .send({
            nonce: web3.utils.toHex(nonce++),
            from: mainAccount.address,
